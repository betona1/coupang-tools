...
### 5-1. 가구매자 리스트 가져오기 → 처리중 (UI 흐름)

가구매 페이지 상단 **🎯 치트키 가구매방** 박스에서 진행:

1. **`📥 가져오기`** — 열린 방(또는 내가 상품등록한 방)에서 상품·구매자를 실시간 크롤링.
   - **📦 나의 상품정보**: 상품별 배송타입(🚀로켓/📦일반배송) + 가구매자 수.
   - **가구매자 리스트**: 상품/옵션/배송/수량/금액/구매자/계좌(은행·번호·예금주)/처리.
2. **처리중 넘기기** — 행별 `처리중 ↪` 또는 `↪ 전체 가구매 처리중으로`.
   - 각 구매자를 수동입력(`site_name='치트키가구매'`)으로 생성 → **가구매목록 > 🕗처리중** 탭에 반영.
   - 계좌정보는 **입금메모**(`은행 계좌숫자 예금주`)에 자동 세팅.
3. **중복 방지** — `가져오기` 시 기존 처리중/완료 목록과 대조.
   - 중복키 = `구매자명 | 계좌숫자 | 상품명`. 이미 넘긴 구매자는 `✓넘김`으로 표시·스킵.
   - **구매자가 늘어나 다시 가져와도 신규만 추가**, 기존건은 중복 생성되지 않음.

### 5-2. 계좌 저장 / 엑셀 (우리은행 대량이체)

- **`💾 계좌 저장`** — 구매자 계좌를 `coupang_gagumae_buyer`에 UPSERT (`?save=1`).
- **`📥 엑셀`** — 우리은행 대량이체 양식 `.xls`(BIFF8) 다운로드. 헤더 없음, 5컬럼:
  `은행 | 계좌번호(숫자만) | 금액 | 예금주 | 통장표시`. 계좌 없는 구매자는 제외.

```python
g.get_buyer_list(room_id=13)   # 배송타입·계좌 포함 리스트
g.save_buyers(room_id=13)      # coupang_gagumae_buyer 저장(UPSERT)
```

**API**: `GET /fake-purchase/gagumae/buyers/?room_id=<id>[&save=1]`
